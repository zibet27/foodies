...
**Note**: The application is configured to use `foodies.localhost` as the hostname. On macOS, `*.localhost` resolves to `127.0.0.1` without `/etc/hosts`.

Pods cannot use that loopback address. Inside the cluster, `foodies.localhost` must resolve to the NGINX ingress 
controller ClusterIP so OIDC discovery uses the same issuer URL as browser redirects (`AUTH_ISSUER`, 
Keycloak `KC_HOSTNAME`, and ingress host must all match).
...
2. Add a `hosts` block to the CoreDNS Corefile. Open the ConfigMap:

```bash
kubectl edit configmap coredns -n kube-system
```

Inside the `.:53 { ... }` server block, add (replace `<INGRESS_IP>` with the ClusterIP from step 1):

```
    hosts {
       <INGRESS_IP> foodies.localhost
       fallthrough
    }
```

Place it before the `kubernetes` plugin. Example:
...
```bash
kubectl run -n foodies dns-check --rm -i --restart=Never --image=curlimages/curl -- \
  curl -sS -o /dev/null -w "%{http_code} %{remote_ip}\n" \
  http://foodies.localhost/auth/realms/foodies-keycloak/.well-known/openid-configuration
```

Expected: HTTP `200` and `remote_ip` equal to the ingress ClusterIP.

If webapp, basket, or order pods crash on startup with OIDC discovery errors after redeploying, confirm this CoreDNS entry exists and matches the current ingress ClusterIP.
...
- **Jaeger UI**: http://foodies.localhost/jaeger (distributed tracing)
...
- **Prometheus UI**: http://foodies.localhost/prometheus (metrics)
...
- **RabbitMQ Management**: http://foodies.localhost:15672 (message broker UI)
...
1. **foodies-config** (generated by Kustomize):
   - `FOODIES_HOST`: Application hostname (dev: `foodies.localhost`, prod: `foodies.com`)
...
└── overlays/                  # Environment-specific overlays
    ├── dev/                   # Development overlay (host: foodies.localhost + storage tweaks)
...
### Hostname / CoreDNS Issues

Symptoms: webapp, basket, or order pods in `CrashLoopBackOff`; logs mention OIDC discovery or `foodies.localhost` connection failures.

```bash
# Confirm ingress is running
kubectl get svc -n ingress-nginx ingress-nginx-controller

# Check CoreDNS has the hosts entry
kubectl get configmap coredns -n kube-system -o yaml | grep -A3 'hosts {'

# Test resolution and OIDC discovery from a pod
kubectl run -n foodies dns-check --rm -i --restart=Never --image=curlimages/curl -- \
  curl -sS -o /dev/null -w "%{http_code} %{remote_ip}\n" \
  http://foodies.localhost/auth/realms/foodies-keycloak/.well-known/openid-configuration
```

If the `remote_ip` is `127.0.0.1` or the request fails, (re)configure CoreDNS as described in [Configure CoreDNS](#configure-coredns). Update the `hosts` entry when the ingress ClusterIP changes.
...
3. Check Prometheus targets:
   - Access Prometheus UI: http://foodies.localhost/prometheus
   - Navigate to **Status** -> **Targets**
   - All service endpoints should be "UP"

4. Verify Jaeger traces:
   - Access Jaeger UI: http://foodies.localhost/jaeger
...
kubectl exec -n foodies -it deployment/webapp -- nslookup foodies.localhost
